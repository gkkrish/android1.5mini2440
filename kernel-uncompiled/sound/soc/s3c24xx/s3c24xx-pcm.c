...
static void init_iis(void){
	...
	writel((S3C2410_IISFCON_TXDMA      /* Transmit FIFO access mode: DMA */
  		| S3C2410_IISFCON_TXENABLE)      /* Transmit FIFO enable */
		,iis_regs + S3C2410_IISFCON);
	
	tmpReg = readl(iis_regs + S3C2410_IISCON);
	printk("iiscon=%08x\n",tmpReg);
	tmpReg |= S3C2410_IISCON_IISEN;
	writel(tmpReg,iis_regs + S3C2410_IISCON);
	tmpReg = readl(iis_regs + S3C2410_IISCON);
	printk("iiscon=%08x\n",tmpReg);
...
static int s3c24xx_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	printk("pcm_prepare\n");
	printk("format=%d,rate=%d,channels=%d\n",substream->runtime->format,substream->runtime->rate,substream->runtime->channels);
