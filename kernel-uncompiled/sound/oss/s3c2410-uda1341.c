...
//printk("audio_write : start count=%d\n", count); 
//return count;
switchMode(PLAYBACK);
...
int i,j;
if(audio_channels == 1)count = count * 2;
//-------------------------------------------------------------------------------
...
uda1341_l3_data(EXTDATA(EXT2_MIC_GAIN(0x6)) | EXT2_MIXMODE_CH2);//input channel 2 select(input channel 1 off) 
...
//printk("init_s3c2410_iis_bus_rx\n"); 
//Kill everything... 
writel(0, iis_base + S3C2410_IISPSR); 
writel(0, iis_base + S3C2410_IISCON); 
writel(0, iis_base + S3C2410_IISMOD); 
...
//printk("init_s3c2410_iis_bus_tx\n"); 
//Kill everything... 
writel(0, iis_base + S3C2410_IISPSR); 
writel(0, iis_base + S3C2410_IISCON); 
writel(0, iis_base + S3C2410_IISMOD); 
