...
{ 
 ...
 printk("audio_dmain_done_callback,size=%d\n",size); 
...
static void showiisregs(void){
	...
	iisfifo = readl(iis_base + S3C2410_IISFIFO);
...
uda1341_l3_data( STAT0_SC_384FS | STAT0_IF_MSB); // reset uda1341 
 uda1341_l3_data(STAT1 | STAT1_ADC_ON | STAT1_DAC_ON); 
// uda1341_l3_data(STAT1 | STAT1_DAC_ON); 
